...
export default function TodoList() {
    ...
    const handleSubmit = (e) => {
        ...
        if (idEdit) {
            ...
            const newList = todos.map(t => t.id === editTodo.id ? (t = { ...t, title: input }) : (t = { ...t }));
            setTodos(newList);
            localStorage.setItem("todos", JSON.stringify(newList));
            inputRef.current.focus();
            setIdEdit(null);
        ...
        // if (idEdit) {
        //     const editList = todos.find((item, i) => i === idEdit);
        //     // console.log(editList);
        //     const updateList = todos.map((l) => l.id === editList.id ? l = { id: l.id, status: l.status, name: list }
    ...
    const handleCheck = (id) => {
        const newList = todos.map(t => t.id === id ? (t = { ...t, status: !t.status }) : (t = { ...t }));
        setTodos(newList);
        localStorage.setItem("todos", JSON.stringify(newList));
        inputRef.current.focus();
    }
                                    ...
                                    <div className="tab-content">
                                        <div className="tab-pane fade show active">
                                          ...
                                          rounded justify-content-between"
                                                    ...
                                                    >
                                                        {t.status === true ? (<s>{t.title}</s>) : (<span>{t.title}</span>)}
                                                        <div>
                                                            <input
                                                                className="form-check-input me-2"
                                                                type="checkbox" checked={t.status === true} onChange={() => handleCheck(t.id)}
                                                            />
                                            ...
                                            <div className="tab-content">
                                                <p style={{ marginTop: 50 }}>Add to the todo list</p>
